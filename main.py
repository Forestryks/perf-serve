...
def display_url_via_ip(ip: str, port: int, prefix: str, name: str, hyperlinks_enabled: bool):
...
def try_display_public(ips: List[Tuple[str, str]], port: int, hyperlinks_enabled: bool):
    public_ip = get_my_ip()
    if public_ip is not None:
        remove_ip_from_list(ips, public_ip)
        display_url_via_ip(public_ip, port, "🚀", f"via public ip ({public_ip})", hyperlinks_enabled)


def try_display_vpn(ips: List[Tuple[str, str]], port: int, hyperlinks_enabled: bool):
    vpn_ip = find_vpn_ip(ips)
    if vpn_ip is not None:
        remove_ip_from_list(ips, vpn_ip)
        display_url_via_ip(vpn_ip, port, "🚀", f"via VPN ({vpn_ip})", hyperlinks_enabled)


def display_possible_urls(port: int, hyperlinks_enabled: bool):
    ips = get_local_ips()
    try_display_public(ips, port, hyperlinks_enabled)
    try_display_vpn(ips, port, hyperlinks_enabled)

    if len(ips) > 0:
        print("Other variants:")
        for ip in ips:
            display_url_via_ip(ip[1], port, "➤", f"via {ip[1]}", hyperlinks_enabled)
...
def main():
    ...
    print()
    display_possible_urls(http_server.server_address[1], args.hyperlinks_enabled)
